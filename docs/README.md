---
title: EEC 315/316 Electronics Laboratory
---

# EEC 315 / 316 Electronics Laboratory

Welcome to the home page for the **Electronics Laboratory** at  
**Cleveland State University – Department of Electrical Engineering and Computer Science**.

This site provides access to the **EEC 315 / 316 Electronics Laboratory Manual (Dec 2021)** and related resources. :contentReference[oaicite:5]{index=5}

> The manual complements the lecture material in EEC 313 (Electronics I) and EEC 314 (Electronics II) and is organized into three parts:
> diodes, transistors, and operational amplifiers.

---

## Download the manual

📄 **Electronics Laboratory Manual (Dec 2021)**  
[Download PDF](EEC315-316-lab-manual-Dec2021.pdf)

---

## Contents of the manual

- **Part I – Diodes**
  - Lab 1: Lab equipment, breadboard, instruments, and PSpice basics
  - Lab 2: Diode rectifier circuits (half-wave and full-wave)
  - Lab 3: Capacitor input rectifier filter
  - Lab 4: Zener diode and voltage regulation

- **Part II – Transistors**
  - Introduction to Analog Discovery 2 and BJT circuits
  - Biasing, small-signal amplifiers, and power amplifiers

- **Part III – Operational Amplifiers**
  - Ideal op-amp concepts, slew rate, CMRR
  - Inverting / non-inverting amplifiers, comparators
  - Differentiator, integrator, and 2nd-order Butterworth filters

---

## Acknowledgments

This manual was developed with support from the  
**Ohio Space Grant Consortium (OSGC) Curriculum Innovation Proposal (CIP)** and reflects contributions from  
**Prof. Chansu Yu, Dr. Lili Dong, Ms. Anusree Mandali, and Mr. Joel Folkmann**. :contentReference[oaicite:6]{index=6}

---

## Contributing

If you use or adapt these labs at your institution, feel free to:

- Open an **Issue** to share feedback or error corrections.
- Submit a **Pull Request** with suggested improvements.

